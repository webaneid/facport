...
## Known Limitations
- **Belum diverifikasi end-to-end lewat UI:** klik "Hubungkan Sekarang" → Accurate → kembali → pilih database di popup. Penyebab: butuh menambahkan redirect URI lewat-proxy di portal developer Accurate (perubahan konfigurasi akun; menunggu izin user) — API-nya sudah terbukti dengan Accurate asli di Fase 143, dan popup/aksi teruji dengan tes komponen.

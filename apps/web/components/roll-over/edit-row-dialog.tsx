...
// DEFAULT ADR-0011 by "No Trans" (opsional).
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `rollOverMapping` di
// `apps/api/src/lib/import-mapping/roll-over.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "serialExpDate", "attributTanggal1", "attributTanggal2"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
export const REQUIRED_INTERNAL_FIELDS = new Set(["transDate", "branchName", "jobOrderNumber", "rollOverType"]);

const FIELD_HINTS: Record<string, string> = {
  rollOverType: "Barang atau Akun — menentukan baris ini masuk ke detail barang atau detail biaya",
  jobOrderNumber: "Nomor Job Order di Accurate — semua baris 1 dokumen harus Job Order yang sama",
  accountNo: "Wajib bila Tipe = Akun, contoh: 5-1001",
  expenseAmount: "Wajib bila Tipe = Akun, contoh: 250000 (angka saja)",
  itemNo: "Wajib bila Tipe = Barang. Kode Barang jadi (FG) di Accurate — TIDAK dibuatkan otomatis",
  quantity: "Wajib bila Tipe = Barang, contoh: 10 (angka saja)",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["roll-over"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

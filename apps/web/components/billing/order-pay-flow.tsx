...
// § Fase 27, ADR-0025 — kontrak MINIMAL yang dibutuhkan flow ini dari
// Eden Treaty binding manapun (`api.orders({id})` versi login, ATAU
// `api.public.orders({id})` versi publik tanpa login) — cukup struktural
// (bukan tipe Eden literal), supaya 1 komponen ini dipakai KEDUA
// halaman TANPA duplikasi ~250 baris JSX/logic yang bisa drift (§ pola
// sama `lib/order-payment.ts` di backend).
export type OrderApiBinding = {
  get: () => Promise<{ data?: unknown; error?: { value?: unknown } | null }>;
  method: { patch: (body: { method: "bank_transfer" | "qris"; accountRef: string }) => Promise<{ data?: unknown; error?: unknown }> };
  qris: { get: () => Promise<{ data?: unknown; error?: unknown }> };
  proof: { patch: (body: { file: File; transferDate: string; payerNote?: string }) => Promise<{ data?: unknown; error?: { value?: unknown } | null }> };
};

// § Fase 16, ADR-0022 (versi login) + Fase 27, ADR-0025 (versi publik
// tanpa login, § `app/landing/pay/[orderId]/page.tsx`) — pilih metode
// (transfer bank/QRIS) -> lihat instruksi (nominal SUDAH termasuk kode
...
export function OrderPayFlow({ orderApi, backHref }: { orderApi: OrderApiBinding; backHref?: string }) {
  ...
  async function load() {
    ...
    const data = res.data as OrderDetail;
    setDetail(data);
    if (data.order.method === "qris") {
      const qrisRes = await orderApi.qris.get();
      if (qrisRes.data) setQris(qrisRes.data as { type: "static" | "dynamic"; imageUrl?: string; qrDataUrl?: string });
    }
  }
  ...
  async function handleSelectMethod(method: "bank_transfer" | "qris") {
    ...
    const res = await orderApi.method.patch({ method, accountRef: selectedAccountRef });
  ...
  async function handleUploadProof() {
    ...
    setUploading(true);
    const res = await orderApi.proof.patch({
      file,
      // § BUG ditemukan 2026-09-06 (audit timezone menyeluruh) — `new
      // Date(transferDate).toISOString()` parse "YYYY-MM-DD" sebagai UTC
      // midnight, bukan tanggal itu di timezone perusahaan. Pakai
      // `middayInTimezone` (§ lib/timezone.ts) supaya tanggal kalender
      // yang tersimpan TIDAK PERNAH bergeser, apa pun timezone yang
      // dipakai buat menampilkannya ulang nanti.
      transferDate: middayInTimezone(transferDate, companyTimezone).toISOString(),
  ...
  if (detail === "not-found") {
    return (
      <div className="mx-auto max-w-2xl">
        <EmptyState icon={Landmark} title="Pembayaran tidak ditemukan" description="Order ini tidak ada atau linknya sudah tidak berlaku." />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      {order.status === "paid" && (
        <Card>
          <CardContent className="py-6">
            <StatusBadge domain="order" status="paid" className="mb-2" />
            <p className="text-sm text-muted-foreground">Pembayaran sudah dikonfirmasi admin. Langganan sudah aktif.</p>
            {backHref && (
              <Button onClick={() => router.push(backHref)} className="mt-3">
                Kembali ke Tagihan
              </Button>
            )}
          </CardContent>
      ...
      {order.status === "submitted" && (
        <Card>
          <CardContent className="py-6">
            <StatusBadge domain="order" status="submitted" className="mb-2" />
          ...
          {order.status === "rejected" && order.rejectionNote && (
            <Card>
              <CardContent className="py-4">
                <StatusBadge domain="order" status="rejected" className="mb-2" />

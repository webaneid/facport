...
// § Fase 113 — dipecah dari `page.tsx` (Server Component) supaya bisa
// terima `dataUsahaId` sebagai prop dan meneruskannya ke `/me/import-batches`
...
export function ImportArchiveView({ dataUsahaId, isDataUsahaOwner }: { dataUsahaId: string; isDataUsahaOwner: boolean }) {
  ...
  async function load() {
    const res = await api.me["import-batches"].get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE, dataUsahaId } });
    if (res.data) {

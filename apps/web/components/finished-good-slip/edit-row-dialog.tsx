...
// § HARUS SINKRON dengan `finishedGoodSlipMapping` di `apps/api/src/lib/import-mapping/finished-good-slip.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "serialExpDate"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
// § KOSONG SENGAJA (mirror Work Order/Material Slip): header dokumen (tanggal, cabang, Work Order No) boleh hanya diisi
// di baris PERTAMA grup, dan baris "lanjutan" nomor seri legit TIDAK punya itemNo/quantity/portion sendiri (§
// `finishedGoodSlipRowError` — continuation-row aware). Menandai kolom ini "wajib" di sini akan salah memblokir baris
// lanjutan yang sah saat diedit sendirian (server yang menilai, lewat `finishedGoodSlipRowError`).
export const REQUIRED_INTERNAL_FIELDS = new Set<string>();

const FIELD_HINTS: Record<string, string> = {
  workOrderNumber: "Nomor Work Order yang sudah ada di Accurate",
  itemNo: "Kode barang jadi di Accurate — TIDAK dibuatkan otomatis. Wajib kecuali baris ini hanya lanjutan Serial No",
  quantity: "Wajib diisi bersama Item No dan Portion (angka saja)",
  portion: "Porsi penyelesaian dalam persen (0-100), wajib diisi bersama Item No dan Qty",
  branchName: "Nama cabang PERSIS seperti di Accurate — wajib, di-lookup ID",
  warehouseName: "Nama gudang PERSIS seperti di Accurate — wajib, di-lookup ID",
};
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["finished-good-slip"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

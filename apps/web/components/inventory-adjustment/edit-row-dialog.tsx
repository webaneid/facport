...
// DEFAULT ADR-0011 by "No. Item Adjustment" (opsional).
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `inventoryAdjustmentMapping` di
// `apps/api/src/lib/import-mapping/inventory-adjustment.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "serialExpDate", "attributTanggal1", "attributTanggal2"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
export const REQUIRED_INTERNAL_FIELDS = new Set(["transDate", "branchName", "itemNo", "itemAdjustmentType", "quantity"]);

const FIELD_HINTS: Record<string, string> = {
  itemAdjustmentType: "Tambah, Kurang, atau Stok Opname",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["inventory-adjustment"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

...
// Validasi "Return Type" (invoiceNumber/deliveryOrderNumber kondisional)
// dicek SERVER via `returnTypeRowError` — kalau field itu error, server
// balikin nama field internal ("invoiceNumber"/"deliveryOrderNumber"/
// "returnType") di response MISSING_REQUIRED_VALUES, resolve otomatis
// lewat `fieldToColumn` yang SUDAH ada di komponen ini.
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `salesReturnMapping` di
// `apps/api/src/lib/import-mapping/sales-return.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "taxDate", "attributHeaderTanggal1", "attributHeaderTanggal2", "itemSerialExpDate"]);
...
const FIELD_HINTS: Record<string, string> = {
  customerNo: "Kode Customer di Accurate, contoh: C.0001 — TIDAK dibuatkan otomatis kalau belum ada",
  returnType: "Isi salah satu: DELIVERY, INVOICE, INVOICE_DP, atau NO_INVOICE",
  invoiceNumber: "WAJIB kalau Return Type = INVOICE atau INVOICE_DP",
  deliveryOrderNumber: "WAJIB kalau Return Type = DELIVERY",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["sales-return"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

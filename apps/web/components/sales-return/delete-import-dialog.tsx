...
// § mirror `components/purchase-return/delete-import-dialog.tsx` —
// hapus batch+baris LOKAL saja, TIDAK PERNAH menyentuh Accurate.
...
export function DeleteImportDialog({ batch, onDeleted }: { batch: DeletableBatch; onDeleted?: () => void }) {
  ...
  async function handleDelete() {
    setSubmitting(true);
    const res = await api["sales-return"].import({ batchId: batch.id }).delete();
    setSubmitting(false);
    if (res.error) {

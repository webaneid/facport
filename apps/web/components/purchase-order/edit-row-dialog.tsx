...
// § mirror `components/sales-receipt/edit-row-dialog.tsx` — modul ini
// juga TIDAK ada grouping-per-faktur-existing (create-only), tapi TETAP
// punya grouping multi-baris by "Trans No" (beda dari Sales Receipt yang
// grouping-nya opsional) — TIDAK ada peringatan "siblingRowNumbers" di
// sini karena edit baris gagal itu genuinely per-baris independen
// (retry cuma proses ulang baris gagal itu sendiri).
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `purchaseOrderMapping` di
...
const FIELD_HINTS: Record<string, string> = {
  ...
  number: "Nomor transaksi — WAJIB, sekaligus kunci penggabungan baris jadi 1 Purchase Order",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["purchase-order"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

...
// § mirror `components/item-transfer/edit-row-dialog.tsx` — modul ini
// juga create-only (TIDAK ada findExisting/append), grouping multi-baris
// DEFAULT ADR-0011 by "Trans No" (opsional).
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `workOrderMapping` di `apps/api/src/lib/import-mapping/work-order.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "startDate", "endDate", "manualFinalDate", "matStdCostDate", "expStdCostDate"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
// § KOSONG SENGAJA: header dokumen (tanggal, akun, BOM, cabang, produk) boleh hanya diisi di baris PERTAMA grup, jadi tidak ada kolom yang
// wajib per baris. Yang divalidasi server per baris: tipe dikenali, section yang terisi lengkap, angka numerik (`workOrderRowError`);
// kelengkapan header dicek per grup saat import berjalan.
export const REQUIRED_INTERNAL_FIELDS = new Set<string>();

const FIELD_HINTS: Record<string, string> = {
  workOrderType: "Kode Produk, Nomor Formula, atau Nomor Rencana Produksi",
  billOfMaterialNo: "Nomor formula (BOM) yang sudah ada di Accurate",
  branchName: "Nama cabang PERSIS seperti di Accurate — harus sudah ada",
  personInChargeName: "Nama penanggung jawab — dibuatkan otomatis kalau belum ada",
  matItemNo: "Wajib diisi bersama Qty kalau baris ini berisi bahan baku",
  matQuantity: "Wajib diisi bersama Item No (angka saja)",
  expItemNo: "Kode item/jasa biaya produksi, BUKAN nomor akun",
  expQuantity: "Wajib diisi bersama Expense No (angka saja)",
  fgItemNo: "Wajib diisi bersama Qty dan Portion kalau ada produk sampingan",
  fgQuantity: "Wajib diisi bersama Extra FG: Item No (angka saja)",
  fgPortion: "Porsi dalam persen, wajib diisi bersama Extra FG: Item No",
};
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["work-order"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

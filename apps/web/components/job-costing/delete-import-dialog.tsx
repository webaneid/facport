...
// § mirror `components/inventory-adjustment/delete-import-dialog.tsx` —
// hapus batch+baris LOKAL saja, TIDAK PERNAH menyentuh Accurate (baik
// Job Order MAUPUN Material Adjustment yang sudah terlanjur dibuat).
...
export function DeleteImportDialog({ batch, onDeleted }: { batch: DeletableBatch; onDeleted?: () => void }) {
  ...
  async function handleDelete() {
    setSubmitting(true);
    const res = await api["job-costing"].import({ batchId: batch.id }).delete();
    setSubmitting(false);
    if (res.error) {
      ...
      <DialogContent>
        ...
        <div className="mt-3 flex flex-col gap-3 text-sm">
          <p className="text-muted-foreground">
            ...
            berhasil masuk Accurate (Job Order dan/atau realisasi bahan baku), transaksinya di Accurate TETAP ADA,
            tapi Facport tidak akan lagi punya catatan bahwa import ini yang membuatnya.
          </p>

...
// § mirror `components/inventory-adjustment/edit-row-dialog.tsx` — modul
// ini juga create-only, grouping multi-baris DEFAULT ADR-0011 by "No.
// Job Order" (opsional). 2 panggilan API (job-order + material-adjustment)
// murni detail backend — TIDAK terlihat di form edit 1 baris ini.
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `jobCostingMapping` di
// `apps/api/src/lib/import-mapping/job-costing.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "serialExpDate"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
export const REQUIRED_INTERNAL_FIELDS = new Set(["transDate", "branchName", "jobAccountNo", "rmItemNo", "rmQty"]);

const FIELD_HINTS: Record<string, string> = {
  jobAccountNo: "Kode akun (COA) pekerjaan — dipakai ulang juga sebagai akun penyesuaian bahan baku",
  rmItemNo: "Kode bahan baku di Accurate, contoh: BRG-001 — TIDAK dibuatkan otomatis kalau belum ada",
  rmQty: "Contoh: 10 (angka saja)",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["job-costing"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

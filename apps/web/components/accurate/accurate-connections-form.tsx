...
// § Fase 113 — dipecah dari `page.tsx` (Server Component) supaya bisa baca
// `dataUsahaId` dari cookie (§ `getActiveDataUsahaIdCookie`, Client
// Component tidak bisa akses `next/headers`) dan teruskan ke tiap fetch —
// SEBELUM ini halaman langsung fetch tanpa scoping Data Usaha sama sekali
// (bug, lihat `docs/phases/phase-113-scoping-data-usaha-dashboard.md`).
export function AccurateConnectionsForm({ dataUsahaId }: { dataUsahaId: string }) {
  ...
  const load = useCallback(async () => {
    const [subsRes, connRes] = await Promise.all([
      api.accurate.subscriptions.get({ query: { dataUsahaId } }),
      api.accurate.connections.get({ query: { dataUsahaId } }),
    ]);
...
function SubscriptionConnectionCard({
  ...
  // § Fase 114 — `reconnect: !!row.accurateConnectionId`: true kalau
  // subscription ini SUDAH pernah punya koneksi (kartu sehat/rusak minta
  // reuse, § picker di bawah dipicu dari 3 status kartu sekarang, bukan
  // cuma first-connect) — backend `POST /accurate/reuse` WAJIB tahu ini
  // supaya tidak tolak 409 ALREADY_CONNECTED (§ accurate.route.ts).
  async function handleReuse() {
    if (!selectedConnectionId) return;
    setReusing(true);
    setError(null);
    const res = await api.accurate.reuse.post({
      subscriptionId: row.subscriptionId,
      connectionId: selectedConnectionId,
      reconnect: !!row.accurateConnectionId,
    });
    setReusing(false);
    if (res.error) {
    ...
    // § security review Fase 114 (Low, DIPERBAIKI) — reset SEBELUM
    // `onChanged()` (refetch), cegah radio button ter-pre-select dari
    // state lama kalau picker ini dibuka lagi buat kartu yang sama nanti.
    setSelectedConnectionId(null);
    onChanged();
  }

  // § Fase 114 — picker reuse DIPINDAH ke sini (top-level, sebelum
  // branching status kartu) supaya bisa dipicu dari SEMUA status
  // (sehat/rusak/belum ada koneksi), bukan cuma "belum ada koneksi sama
  // sekali" seperti sebelumnya — temuan debugging production 2026-09-14:
  // tombol "Hubungkan Ulang" SELALU OAuth baru, numpuk koneksi ke company
  // yang SAMA (2 customer nyata sampai 5-17 koneksi terpisah, §
  // lessons-learned.md). Konten JSX-nya SAMA PERSIS seperti before,
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            {/* § Fase 114 — kalau ada koneksi LAIN yang sudah aktif di Data
                Usaha ini (modul lain sudah reconnect duluan), tawarkan
                reuse dulu — jangan langsung paksa OAuth baru yang bikin
                koneksi numpuk ke company yang sama. */}
            {connections.length > 0 && (
              <Button variant="outline" size="sm" className="w-fit" onClick={() => setShowReusePicker(true)}>
    ...
    return (
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          ...
          {/* § Fase 114 — sama alasan kartu sehat di atas: tawarkan reuse
              dulu kalau ada koneksi lain yang masih aktif di Data Usaha ini. */}
          <div className="flex flex-wrap gap-2">
            {connections.length > 0 && (
  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-3">
        {/* § Fase 114 — picker reuse sekarang di-render lewat early-return
            `showReusePicker` di atas (dipakai bareng kartu sehat/rusak),
            jadi cabang ini cuma render tombol pemicu, tidak perlu ternary
            lagi. */}
        <div className="flex flex-wrap gap-2">
          {connections.length > 0 && (
            <Button variant="outline" onClick={() => setShowReusePicker(true)}>

...
// juga punya grouping multi-baris by "Trans Number" (opsional, pola
// DEFAULT ADR-0011) — TIDAK ada peringatan "siblingRowNumbers" di sini
...
// § HARUS SINKRON dengan `salesQuotationMapping` di
// `apps/api/src/lib/import-mapping/sales-quotation.mapping.ts`.
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["sales-quotation"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

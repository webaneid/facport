...
// ADR-0011) — TIDAK ada peringatan "siblingRowNumbers" di sini karena
// edit baris gagal itu genuinely per-baris independen.
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `salesOrderMapping` di
// `apps/api/src/lib/import-mapping/sales-order.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "shipDate"]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);
export const REQUIRED_INTERNAL_FIELDS = new Set(["customerNo", "transDate", "itemNo", "unitPrice", "quantity", "itemUnitName", "branchName"]);

const FIELD_HINTS: Record<string, string> = {
  customerNo: "Kode Customer di Accurate, contoh: C.0001 — dibuatkan otomatis kalau belum ada",
  itemNo: "Kode Barang di Accurate, contoh: BRG-001 — dibuatkan otomatis kalau belum ada",
  ...
  salesmanListNumber: "Boleh lebih dari 1, pisahkan dengan koma (contoh: SLS-01,SLS-02)",
};
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["sales-order"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

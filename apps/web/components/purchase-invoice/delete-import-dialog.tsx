...
// Excel & record Facport), TIDAK PERNAH menyentuh Accurate — beda dari
// "Batal Import" (`cancel-import-dialog.tsx`, Fase 09) yang menghapus
// transaksi Accurate. Konfirmasi klik biasa (BUKAN type-to-confirm
// seperti Batal Import) — risikonya lebih rendah (tidak menyentuh data
// akuntansi asli pihak ketiga), tapi TETAP permanen jadi tetap perlu
// 1 langkah konfirmasi eksplisit.
...
export function DeleteImportDialog({ batch, onDeleted }: { batch: DeletableBatch; onDeleted?: () => void }) {
  ...
  async function handleDelete() {
    setSubmitting(true);
    const res = await api["purchase-invoice"].import({ batchId: batch.id }).delete();
    setSubmitting(false);
    if (res.error) {

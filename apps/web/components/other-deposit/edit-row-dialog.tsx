...
// § HARUS SINKRON dengan `otherDepositMapping` di
...
const FIELD_HINTS: Record<string, string> = {
  transNo: "Nomor transaksi — kunci pengelompokan baris dalam 1 penerimaan, contoh: OD-001",
  branchName: "Nama cabang, contoh: JAKARTA",
  bankNo: "Kode Akun (COA) kas/bank tujuan dana masuk, contoh: 1-10200",
  payee: "Informasi pemberi/sumber dana, contoh: PT Mitra Jaya",
  ...
  lineAccountNo: "Kode Akun (COA) baris ini, contoh: 4-10100",
  lineAmount: "Nominal baris ini — angka saja, tanpa titik/koma",
  lineExpenseName: "Nama/keterangan baris ini (field API Accurate literal \"expenseName\") — BUKAN nama master akun, contoh: Pendapatan Lain-lain",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["other-deposit"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

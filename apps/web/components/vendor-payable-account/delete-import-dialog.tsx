...
// § mirror `components/purchase-invoice/delete-import-dialog.tsx` — hapus
// batch+baris LOKAL saja, TIDAK PERNAH menyentuh Accurate.
...
export function DeleteImportDialog({ batch, onDeleted }: { batch: DeletableBatch; onDeleted?: () => void }) {
  ...
  async function handleDelete() {
    setSubmitting(true);
    const res = await api.vendor["payable-account"].import({ batchId: batch.id }).delete();
    setSubmitting(false);
    if (res.error) {

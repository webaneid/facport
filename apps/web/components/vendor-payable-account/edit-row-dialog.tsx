...
// § modul ini TIDAK punya field tanggal (requiredFields:
// vendorPayableAccountMapping) — Set kosong, TIDAK perlu normalisasi
// serial-Excel seperti Purchase Invoice.
export const DATE_INTERNAL_FIELDS = new Set<string>([]);
const EXCEL_EPOCH_UTC_MS = Date.UTC(1899, 11, 30);

// § HARUS SINKRON dengan `vendorPayableAccountMapping.requiredFields` di
// `apps/api/src/lib/import-mapping/vendor-payable-account.mapping.ts`.
export const REQUIRED_INTERNAL_FIELDS = new Set(["vendorNo", "payableAccountNo"]);

const FIELD_HINTS: Record<string, string> = {
  vendorNo: "Kode Pemasok di Accurate, contoh: V.00001",
  payableAccountNo: "Kode Akun Hutang (COA) di Accurate, contoh: 2-10100",
};
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api.vendor["payable-account"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

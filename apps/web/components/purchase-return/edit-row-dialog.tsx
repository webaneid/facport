...
// juga create-only, grouping DEFAULT ADR-0011 by "TransNo" (opsional).
// Validasi "Return Type" (invoiceNumber/receiveItemNumber kondisional)
// dicek SERVER via `returnTypeRowError` — kalau field itu error, server
// balikin nama field internal ("invoiceNumber"/"receiveItemNumber"/
// "returnType") di response MISSING_REQUIRED_VALUES, resolve otomatis
// lewat `fieldToColumn` yang SUDAH ada di komponen ini, tidak perlu
// logic tambahan di client.
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `purchaseReturnMapping` di
// `apps/api/src/lib/import-mapping/purchase-return.mapping.ts`.
export const DATE_INTERNAL_FIELDS = new Set(["transDate", "taxDate", "attributItemTanggal1", "attributItemTanggal2"]);
...
const FIELD_HINTS: Record<string, string> = {
  vendorNo: "Kode Vendor di Accurate, contoh: V.0001 — TIDAK dibuatkan otomatis kalau belum ada",
  returnType: "Isi salah satu: INVOICE, INVOICE_DP, RECEIVE, atau NO_INVOICE",
  invoiceNumber: "WAJIB kalau Return Type = INVOICE atau INVOICE_DP",
  receiveItemNumber: "WAJIB kalau Return Type = RECEIVE",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["purchase-return"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {
      ...
      if (value?.code === "MISSING_REQUIRED_VALUES") {
        ...
        setError(`${cols.length} kolom wajib belum diisi atau tidak konsisten dengan Return Type — lihat tanda merah di bawah.`);
      } else {
        setMissingColumns(new Set());

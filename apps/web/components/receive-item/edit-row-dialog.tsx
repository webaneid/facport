...
// § mirror `components/purchase-order/edit-row-dialog.tsx` — modul ini
// juga create-only (TIDAK ada findExisting/append), grouping multi-baris
// by "Receive Number" (BEDA dari Purchase Order yang by "Trans No") —
// TIDAK ada peringatan "siblingRowNumbers" di sini karena edit baris
// gagal itu genuinely per-baris independen (retry cuma proses ulang
// baris gagal itu sendiri).
type EditableRow = {
  id: string;
  rowNumber: number;
...
// § HARUS SINKRON dengan `receiveItemMapping` di
// `apps/api/src/lib/import-mapping/receive-item.mapping.ts`.
...
export const REQUIRED_INTERNAL_FIELDS = new Set(["vendorNo", "transDate", "receiveNumber", "itemNo", "unitPrice", "quantity", "itemUnitName", "branchName"]);

const FIELD_HINTS: Record<string, string> = {
  vendorNo: "Kode Vendor di Accurate, contoh: V.0001 — TIDAK dibuatkan otomatis kalau belum ada",
  receiveNumber: "Nomor surat jalan/pengiriman dari VENDOR — WAJIB, sekaligus kunci penggabungan baris jadi 1 Receive Item",
...
export function EditRowDialog({
  ...
  async function handleSave() {
    ...
    setSubmitting(true);
    const res = await api["receive-item"].import({ batchId }).rows({ rowId: row.id }).put({ rawData: values });
    setSubmitting(false);
    if (res.error) {

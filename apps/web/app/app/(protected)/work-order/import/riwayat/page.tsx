...
import { DeleteImportDialog } from "@/components/work-order/delete-import-dialog";
...
// (modul ini tidak punya cancel, § architecture-work-order.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function WorkOrderImportArchivePage() {
  ...
  async function load() {
    const res = await api["work-order"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Work Order, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/work-order/import/${batch.id}`}
                            title="Detail"

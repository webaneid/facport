...
import { DeleteImportDialog } from "@/components/receive-item/delete-import-dialog";
...
// (modul ini tidak punya cancel, § architecture-receive-item.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function ReceiveItemImportArchivePage() {
  ...
  async function load() {
    const res = await api["receive-item"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Receive Item, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/receive-item/import/${batch.id}`}
                            title="Detail"

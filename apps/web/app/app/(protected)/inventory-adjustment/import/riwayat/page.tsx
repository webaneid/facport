...
import { DeleteImportDialog } from "@/components/inventory-adjustment/delete-import-dialog";
...
// § mirror `item-transfer/import/riwayat/page.tsx` — TANPA Batal Import
// (modul ini tidak punya cancel, § architecture-inventory-adjustment.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function InventoryAdjustmentImportArchivePage() {
  ...
  async function load() {
    const res = await api["inventory-adjustment"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Inventory Adjustment, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/inventory-adjustment/import/${batch.id}`}
                            title="Detail"

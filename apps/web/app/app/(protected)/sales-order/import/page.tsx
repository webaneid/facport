...
// Customer/Item auto-create kalau belum ada di Accurate. TIDAK ada
// dampak GL/stok — bukan transaksi akuntansi final.
const ACCURATE_FIELDS = [
  { value: "", label: "(tidak dipetakan)" },
  { value: "transDate", label: "Trans Date (wajib)" },
  { value: "number", label: "Trans No (opsional — kunci gabung baris kalau diisi)" },
  ...
  { value: "salesQuotationNumber", label: "Sales Quot No" },
  ...
  { value: "salesmanListNumber", label: "Sales List No (separate with comma)" },
  ...
  { value: "expenseSalesQuotationNumber", label: "Expense Sales Quot No" },
  { value: "expenseDepartmentName", label: "Expense Dept" },
...
export default function SalesOrderImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["sales-order"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["sales-order"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/sales-order/import/${result.batchId}`);
  }
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi Pesanan Penjualan. Customer dan Barang akan dibuatkan otomatis di Accurate kalau
          belum terdaftar — Sales Order sendiri bukan transaksi akuntansi (tidak ada jurnal, tidak ada perubahan
          stok).
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/sales-order/import/template`}

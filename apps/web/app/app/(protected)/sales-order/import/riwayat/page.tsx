...
import { DeleteImportDialog } from "@/components/sales-order/delete-import-dialog";
...
// § mirror `sales-quotation/import/riwayat/page.tsx` — TANPA Batal
// Import (modul ini tidak punya cancel, § architecture-sales-order.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function SalesOrderImportArchivePage() {
  ...
  async function load() {
    const res = await api["sales-order"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Sales Order, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/sales-order/import/${batch.id}`}
                            title="Detail"

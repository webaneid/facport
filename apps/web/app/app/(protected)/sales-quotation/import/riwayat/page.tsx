...
import { DeleteImportDialog } from "@/components/sales-quotation/delete-import-dialog";
...
// § mirror `purchase-order/import/riwayat/page.tsx` — TANPA Batal Import
// (modul ini tidak punya cancel, § architecture-sales-quotation.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function SalesQuotationImportArchivePage() {
  ...
  async function load() {
    const res = await api["sales-quotation"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Sales Quotation, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/sales-quotation/import/${batch.id}`}
                            title="Detail"

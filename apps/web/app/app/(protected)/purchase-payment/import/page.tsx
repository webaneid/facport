...
export default function PurchasePaymentImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["purchase-payment"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["purchase-payment"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/purchase-payment/import/${result.batchId}`);
  }
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi pembayaran ke Faktur Pembelian. Vendor dan nomor faktur WAJIB SUDAH terdaftar di
          Accurate — proses ini membayar tagihan yang sudah ada, bukan membuat faktur baru.
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/purchase-payment/import/template`}

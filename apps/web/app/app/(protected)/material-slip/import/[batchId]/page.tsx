...
import { EditRowDialog, DATE_INTERNAL_FIELDS, REQUIRED_INTERNAL_FIELDS } from "@/components/material-slip/edit-row-dialog";
...
export default function MaterialSlipImportResultPage() {
  ...
  async function load() {
    const res = await api["material-slip"].import({ batchId: params.batchId }).get();
  ...
  async function handleGridSave(rowsToSave: { id: string; rawData: Record<string, string> }[]) {
    const res = await api["material-slip"].import({ batchId: params.batchId }).rows.put({ rows: rowsToSave });
  ...
  async function handleRetry() {
    setRetrying(true);
    const res = await api["material-slip"].import({ batchId: params.batchId }).retry.post();
    setRetrying(false);
    if (res.error) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Hasil Import Material Slip</h1>
      ...
      <Card>
        ...
        {(summary.failed > 0 || summary.pending > 0) && !isProcessing && batch.columnMapping && (
          <CardContent className="flex flex-wrap items-center gap-3">
            ...
            {summary.failed > 0 && (
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/material-slip/import/${batch.id}/failed-rows/export`}

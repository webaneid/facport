...
// § architecture-material-slip.md, Fase 148 — Material Slip (realisasi bahan baku dari Work Order). Grouping 2-level:
// 1 dokumen ("Trans No") bisa punya BEBERAPA barang, dan 1 barang bisa punya BEBERAPA nomor seri di baris terpisah
// (baris "lanjutan": Item No sama, kolom Qty barang dikosongkan) — § `manufacture-slip-shared.ts`.
const ACCURATE_FIELDS = [
  ...
  { value: "materialSlipType", label: "Material Slip Type (wajib — Pengambilan/Pengembalian)" },
  ...
  { value: "warehouseName", label: "Warehouse Name" },
...
export default function MaterialSlipImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["material-slip"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["material-slip"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/material-slip/import/${result.batchId}`);
  }
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Import Material Slip</h1>
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi Material Slip (realisasi pengambilan/pengembalian bahan baku dari Work Order). Barang dan
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/material-slip/import/template`}

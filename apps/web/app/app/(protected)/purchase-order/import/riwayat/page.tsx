...
import { DeleteImportDialog } from "@/components/purchase-order/delete-import-dialog";
...
// § mirror `sales-receipt/import/riwayat/page.tsx` — TANPA Batal Import
// (modul ini tidak punya cancel, § architecture-purchase-order.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function PurchaseOrderImportArchivePage() {
  ...
  async function load() {
    const res = await api["purchase-order"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Purchase Order, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/purchase-order/import/${batch.id}`}
                            title="Detail"

...
// § architecture-purchase-order.md, Fase 120 — titik AWAL rantai
// procurement (Purchase Order → Receive Item → Purchase Invoice →
// Purchase Payment). Vendor/Item auto-create kalau belum ada di
// Accurate (mirror Purchase Invoice). "Trans No" WAJIB — sekaligus
// kunci penggabungan baris jadi 1 PO berisi banyak barang.
const ACCURATE_FIELDS = [
  ...
  { value: "itemDiscPercent", label: "ITEM: Disc Percent" },
  { value: "purchaseRequisitionNumber", label: "ITEM: Requisite No" },
  { value: "departmentName", label: "ITEM: Department" },
  { value: "projectNo", label: "ITEM: Project No" },
  { value: "useTax1", label: "PPN (isi TRUE/FALSE)" },
  { value: "useTax2", label: "PPnBM (isi TRUE/FALSE)" },
  { value: "useTax3", label: "PPh (isi TRUE/FALSE)" },
  ...
  { value: "expenseProjectNo", label: "EXPENSE: Project No" },
...
export default function PurchaseOrderImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["purchase-order"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["purchase-order"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/purchase-order/import/${result.batchId}`);
  }
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Import Purchase Order</h1>
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi Pesanan Pembelian. Vendor dan Barang akan dibuatkan otomatis di Accurate kalau belum
          terdaftar — Purchase Order sendiri bukan transaksi akuntansi (tidak ada jurnal), murni pesanan/komitmen ke
          vendor.
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/purchase-order/import/template`}

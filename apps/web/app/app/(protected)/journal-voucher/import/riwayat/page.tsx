...
import { DeleteImportDialog } from "@/components/journal-voucher/delete-import-dialog";
...
export default function JournalVoucherImportArchivePage() {
  ...
  async function load() {
    const res = await api["journal-voucher"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Jurnal Umum, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/journal-voucher/import/${batch.id}`}
                            title="Detail"

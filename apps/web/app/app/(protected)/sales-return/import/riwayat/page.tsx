...
import { DeleteImportDialog } from "@/components/sales-return/delete-import-dialog";
...
// § mirror `purchase-return/import/riwayat/page.tsx` — TANPA Batal
// Import (modul ini tidak punya cancel, § architecture-sales-return.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function SalesReturnImportArchivePage() {
  ...
  async function load() {
    const res = await api["sales-return"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Sales Return, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/sales-return/import/${batch.id}`}
                            title="Detail"

...
const ACCURATE_FIELDS = [
  ...
  { value: "quantity", label: "FG Qty (wajib bila Barang)" },
  { value: "itemUnitName", label: "FG Unit" },
  ...
  { value: "accountNo", label: "Expense Acc No (wajib bila Akun)" },
  { value: "expenseAmount", label: "Expense Amount (wajib bila Akun)" },
  { value: "expenseName", label: "Expense Name" },
  { value: "expenseNotes", label: "Expense Note" },
  ...Array.from({ length: 10 }, (_, i) => ({ value: `attributTambahan${i + 1}`, label: `Atribut Tambahan ${i + 1}` })),
  ...Array.from({ length: 10 }, (_, i) => ({ value: `attributNumber${i + 1}`, label: `Atribut Number ${i + 1}` })),
  { value: "attributTanggal1", label: "Atribut Date 1" },
  { value: "attributTanggal2", label: "Atribut Date 2" },
  ...Array.from({ length: 10 }, (_, i) => ({ value: `kategoriKeuangan${i + 1}`, label: `Financial Category ${i + 1}` })),
] as const;
...
export default function RollOverImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["roll-over"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["roll-over"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/roll-over/import/${result.batchId}`);
  }
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Import Roll Over</h1>
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi Roll Over (penyelesaian pesanan produksi). Barang jadi, akun, dan Job Order
          TIDAK dibuatkan otomatis — pastikan sudah ada di Accurate sebelum import.
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/roll-over/import/template`}

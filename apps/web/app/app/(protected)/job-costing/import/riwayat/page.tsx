...
import { DeleteImportDialog } from "@/components/job-costing/delete-import-dialog";
...
// § mirror `inventory-adjustment/import/riwayat/page.tsx` — TANPA Batal
// Import (modul ini tidak punya cancel, § architecture-job-costing.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function JobCostingImportArchivePage() {
  ...
  async function load() {
    const res = await api["job-costing"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Job Costing, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/job-costing/import/${batch.id}`}
                            title="Detail"

...
import { DeleteImportDialog } from "@/components/item-requisition/delete-import-dialog";
...
// § mirror `receive-item/import/riwayat/page.tsx` — TANPA Batal Import
// (modul ini tidak punya cancel, § architecture-item-requisition.md).
type ImportBatch = { id: string; fileName: string; status: string; totalRows: number; createdAt: string };

const PAGE_SIZE = 20;

export default function ItemRequisitionImportArchivePage() {
  ...
  async function load() {
    const res = await api["item-requisition"].import.get({ query: { limit: PAGE_SIZE, offset: page * PAGE_SIZE } });
    if (res.data) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Arsip Riwayat Import" description="Semua import Item Requisition, termasuk yang lebih lama." />
                    ...
                    <TableRow key={batch.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-1">
                          <Link
                            href={`/item-requisition/import/${batch.id}`}
                            title="Detail"

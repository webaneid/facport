...
// digabung jadi 1 transaksi (bisa lebih dari 1 akun). TIDAK ADA
// validasi balance (beda dari Jurnal Umum) — akun kas/bank otomatis
// sisi debit via API.
const ACCURATE_FIELDS = [
  ...
  { value: "payee", label: "Pemberi/Sumber Dana (wajib)" },
  ...
  { value: "lineExpenseName", label: "Nama/Keterangan per Baris (wajib, BUKAN nama master akun)" },
  { value: "lineAmount", label: "Nominal per Baris (wajib)" },
...
export default function OtherDepositImportPage() {
  ...
  async function onUpload(values: UploadValues) {
    ...
    const res = await api["other-deposit"].import.upload.post({ file });
  ...
  async function onConfirmMapping() {
    ...
    const res = await api["other-deposit"].import({ batchId: result.batchId }).confirm.post({ columnMapping });
    setConfirming(false);
    if (res.error) {
    ...
    router.push(`/other-deposit/import/${result.batchId}`);
  }
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Import Other Deposit</h1>
        <p className="text-sm text-muted-foreground">
          Upload file Excel berisi penerimaan kas/bank di luar penjualan (setoran modal, pendapatan lain-lain, dll)
          — TANPA faktur atau customer. Baris dengan Nomor Transaksi yang sama digabung jadi 1 penerimaan (boleh
          lebih dari 1 akun). Kode akun kas/bank dan akun WAJIB SUDAH terdaftar di Accurate.
        </p>
      </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <a
                href={`${process.env.NODE_ENV === "production" ? getProdApiOrigin() : (process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3001")}/other-deposit/import/template`}

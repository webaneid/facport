...
function suggestMapping(excelColumns: string[]): Record<string, string> {
  ...
  for (const col of excelColumns) {
    ...
    const match = Object.keys(rollOverMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = rollOverMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-roll-over.md (Fase 146) — TIDAK auto-create item/akun, TIDAK ADA "Batal Import". `rollOverRowError` (tipe dikenali + field wajib
// per tipe: Barang → FG_Item No/FG_Qty, Akun → Expense Acc No/Amount) divalidasi TERPISAH dari `requiredFields` biasa di KEDUA endpoint edit
// baris gagal. Pengecekan scope Accurate (`checkSubscriptionScopes`) di confirm & retry (Fase 142).
export const rollOverImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/roll-over/import/template",
    () => {
      const buffer = generateTemplateBuffer(rollOverTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-roll-over.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "roll_over" },
  )
  .get(
    "/roll-over/import",
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "roll_over"));
  ...
  .post(
    "/roll-over/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = rollOverMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "roll_over");
  ...
  .post(
    "/roll-over/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/roll-over/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const missing: string[] = rollOverMapping.requiredFields.filter((field) => {
      ...
      for (const f of rollOverRowError(body.rawData, columnMapping)) if (!missing.includes(f)) missing.push(f);
      if (missing.length > 0) {
        set.status = 400;
  ...
  .put(
    ...
    async ({ params, body, subscription, set }) => {
      ...
      for (const item of body.rows) {
        ...
        for (const f of rollOverRowError(item.rawData, columnMapping)) if (!missing.includes(f)) missing.push(f);
        if (missing.length > 0) {

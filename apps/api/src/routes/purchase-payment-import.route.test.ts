...
describe("DELETE /purchase-payment/import/:batchId — hapus riwayat lokal", () => {
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`pp-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "purchase_payment", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `pp-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/purchase-payment/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }),
    );
    expect(res.status).toBe(403);

...
import { workOrderMapping, workOrderRowError } from "../lib/import-mapping/work-order.mapping";
import { workOrderTemplateGuide } from "../lib/import-mapping/template-guide";
...
const VALID_FIELDS = new Set(Object.keys(workOrderMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(workOrderMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = workOrderMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-work-order.md (Fase 147) — TIDAK auto-create item/akun, TIDAK ADA "Batal Import". Edit baris gagal (satu & massal) HANYA
// memakai `workOrderRowError` (tipe dikenali, section yang terisi lengkap, angka numerik) — BUKAN `requiredFields` per baris, karena header
// dokumen boleh hanya diisi di baris PERTAMA grup (kelengkapan header dicek per grup di worker). Pengecekan scope Accurate
// (`checkSubscriptionScopes`) di confirm & retry (Fase 142).
export const workOrderImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/work-order/import/template",
    () => {
      const buffer = generateTemplateBuffer(workOrderTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-work-order.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "work_order" },
  )
  .get(
    "/work-order/import",
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "work_order"));
  ...
  .post(
    "/work-order/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = workOrderMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "work_order");
  ...
  .get(
    "/work-order/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/work-order/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/work-order/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing = workOrderRowError(body.rawData, columnMapping);
      if (missing.length > 0) {
        set.status = 400;
  ...
  .put(
    "/work-order/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      for (const item of body.rows) {
        ...
        const missing = workOrderRowError(item.rawData, columnMapping);
        if (missing.length > 0) {

...
import { purchaseReturnMapping, returnTypeRowError } from "../lib/import-mapping/purchase-return.mapping";
import { purchaseReturnTemplateGuide } from "../lib/import-mapping/template-guide";
import { boss, JOBS } from "../lib/queue";
import { checkTrialRowBudget } from "../lib/trial";

// § architecture-security.md §8, pola sama purchase-order-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(purchaseReturnMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(purchaseReturnMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = purchaseReturnMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-purchase-return.md — dokumen LANJUTAN (TIDAK
// auto-create vendor/item, mirror Receive Item). TIDAK ADA "Batal
// Import" — route ini sesederhana purchase-order-import.route.ts, tanpa
// endpoint cancel. `returnTypeRowError` divalidasi TERPISAH dari
// requiredFields generik (mirror `debitCreditRowError` Journal Voucher)
// karena field companion-nya (invoiceNumber/receiveItemNumber) WAJIB
// KONDISIONAL tergantung nilai returnType, bukan wajib mutlak.
export const purchaseReturnImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/purchase-return/import/template",
    () => {
      const buffer = generateTemplateBuffer(purchaseReturnTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-purchase-return.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "purchase_return" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "purchase_return"));
  ...
  .post(
    "/purchase-return/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = purchaseReturnMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "purchase_return");
  ...
  .get(
    "/purchase-return/import/:batchId",
  ...
  .get(
    "/purchase-return/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/purchase-return/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/purchase-return/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing: string[] = purchaseReturnMapping.requiredFields.filter((field) => {
  ...
  .put(
    "/purchase-return/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

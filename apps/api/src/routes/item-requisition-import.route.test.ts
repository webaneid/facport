...
import { itemRequisitionImportRoute } from "./item-requisition-import.route";
...
async function signUp(email: string) {
  const res = await testApp.handle(
    new Request("http://localhost/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({ email, password: "TestPassword123!", name: "Item Requisition Import Test" }),
    }),
  );
...
async function createProvisionedUser(email: string) {
  ...
  const [plan] = await db
    .insert(plans)
    .values({ name: `Item Requisition Import Test Plan ${email}`, price: 1000, durationDays: 30, modules: ["item_requisition"] })
    .returning();
...
describe("GET /item-requisition/import/template", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/item-requisition/import/template"));
    expect(res.status).toBe(401);
  });

  test("403 kalau login tapi tidak punya permission import.create", async () => {
    const email = `irq-noperm-${runId}@test.local`;
    await signUp(email);
    const cookie = await signIn(email);

    const res = await testApp.handle(new Request("http://localhost/item-requisition/import/template", { headers: { cookie } }));
    expect(res.status).toBe(403);
  });
});

describe("POST /item-requisition/import/upload", () => {
  test("401 kalau tidak login (dengan file .xlsx asli, supaya bukan gagal validasi t.File duluan)", async () => {
    ...
    const res = await testApp.handle(new Request("http://localhost/item-requisition/import/upload", { method: "POST", body: form }));
    expect(res.status).toBe(401);
  });
});

describe("Ownership batch — user lain TIDAK BOLEH akses batch orang lain", () => {
  test("GET/POST /item-requisition/import/:batchId* dengan batchId milik user LAIN → 404, bukan bocor data", async () => {
    const owner = await createProvisionedUser(`irq-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`irq-attacker-${runId}@test.local`);
    ...
    const getRes = await testApp.handle(new Request(`http://localhost/item-requisition/import/${batch!.id}`, { headers: { cookie: attacker.cookie } }));
    expect(getRes.status).toBe(404);
    expect(((await getRes.json()) as { code: string }).code).toBe("BATCH_NOT_FOUND");

    const retryRes = await testApp.handle(
      new Request(`http://localhost/item-requisition/import/${batch!.id}/retry`, { method: "POST", headers: { cookie: attacker.cookie } }),
    );
    expect(retryRes.status).toBe(404);

    const ownerRes = await testApp.handle(new Request(`http://localhost/item-requisition/import/${batch!.id}`, { headers: { cookie: owner.cookie } }));
    expect(ownerRes.status).toBe(200);
  });
});

describe("POST /item-requisition/import/:batchId/confirm — validasi mapping", () => {
  test("400 MISSING_REQUIRED_FIELDS kalau kolom wajib belum di-mapping", async () => {
    const owner = await createProvisionedUser(`irq-confirm-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "mapping_pending" })
      .returning();
  ...
  test("400 INVALID_MAPPING_FIELD kalau ada kolom di-mapping ke field yang tidak dikenal", async () => {
    const owner = await createProvisionedUser(`irq-invalidfield-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "mapping_pending" })
      .returning();
...
describe("GET /item-requisition/import (list)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/item-requisition/import"));
    expect(res.status).toBe(401);
  });

  test("cuma return batch milik subscription sendiri, urut terbaru dulu, dibatasi ?limit", async () => {
    const owner = await createProvisionedUser(`irq-list-owner-${runId}@test.local`);
    const other = await createProvisionedUser(`irq-list-other-${runId}@test.local`);
    ...
    for (const fileName of ["batch-1.xlsx", "batch-2.xlsx", "batch-3.xlsx"]) {
      await db.insert(importBatches).values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName, totalRows: 1, status: "completed" });
    }

    const res = await testApp.handle(new Request("http://localhost/item-requisition/import?limit=2", { headers: { cookie: owner.cookie } }));
  ...
  test("?offset melompati N batch terbaru, `total` tetap hitungan penuh — pola halaman Riwayat", async () => {
    const owner = await createProvisionedUser(`irq-offset-${runId}@test.local`);
    for (const fileName of ["off-1.xlsx", "off-2.xlsx", "off-3.xlsx"]) {
      await db.insert(importBatches).values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName, totalRows: 1, status: "completed" });
    }

    const res = await testApp.handle(new Request("http://localhost/item-requisition/import?limit=2&offset=1", { headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(200);
    const body = (await res.json()) as { batches: { fileName: string }[]; total: number };
...
describe("PUT /item-requisition/import/:batchId/rows/:rowId — Edit Baris", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/item-requisition/import/00000000-0000-0000-0000-000000000000/rows/00000000-0000-0000-0000-000000000000", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`irq-editrow-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`irq-editrow-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
  ...
  test("409 ROW_NOT_EDITABLE kalau baris statusnya bukan failed", async () => {
    const owner = await createProvisionedUser(`irq-editrow-noteditable-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "completed", columnMapping })
      .returning();
  ...
  test("400 MISSING_REQUIRED_VALUES kalau field wajib dikosongkan, sukses (status pending) kalau lengkap", async () => {
    const owner = await createProvisionedUser(`irq-editrow-save-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
...
describe("PUT /item-requisition/import/:batchId/rows — Edit Bulk (Grid)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/item-requisition/import/00000000-0000-0000-0000-000000000000/rows", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`irq-bulkedit-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`irq-bulkedit-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
  ...
  test("campuran baris valid & field wajib kosong, bukan gagalkan seluruh request", async () => {
    const owner = await createProvisionedUser(`irq-bulkedit-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 2, status: "completed_with_errors", columnMapping })
      .returning();
...
describe("DELETE /item-requisition/import/:batchId — hapus riwayat lokal", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/item-requisition/import/00000000-0000-0000-0000-000000000000", { method: "DELETE" }));
    expect(res.status).toBe(401);
  });

  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`irq-delete-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`irq-delete-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/item-requisition/import/${batch!.id}`, { method: "DELETE", headers: { cookie: attacker.cookie } }));
    expect(res.status).toBe(404);
  });
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`irq-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `irq-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(new Request(`http://localhost/item-requisition/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }));
    expect(res.status).toBe(403);
  ...
  test("409 BATCH_BUSY kalau status processing", async () => {
    const owner = await createProvisionedUser(`irq-delete-busy-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "test.xlsx", totalRows: 1, status: "processing" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/item-requisition/import/${batch!.id}`, { method: "DELETE", headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(409);
  ...
  test("berhasil hapus batch + baris (cascade), tercatat di audit log", async () => {
    const owner = await createProvisionedUser(`irq-delete-ok-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "item_requisition", fileName: "hapus-saya.xlsx", totalRows: 1, status: "completed" })
      .returning();

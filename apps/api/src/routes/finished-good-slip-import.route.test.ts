...
// tipe enum; `itemNo`/`quantity`/`portion` WAJIB bersamaan per barang (`finishedGoodSlipRowError`); `branchName`/
// `workOrderNumber` WAJIB di level mapping (beda dari Material Slip yang cabang-nya opsional).
const runId = Date.now();
const testApp = new Elysia().mount(auth.handler).use(finishedGoodSlipImportRoute);
const columnMapping = {
...
async function signUp(email: string) {
  const res = await testApp.handle(
    new Request("http://localhost/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({ email, password: "TestPassword123!", name: "Finished Good Slip Import Test" }),
    }),
  );
...
async function createProvisionedUser(email: string) {
  ...
  const [plan] = await db
    .insert(plans)
    .values({ name: `Finished Good Slip Import Test Plan ${email}`, price: 1000, durationDays: 30, modules: ["finished_good_slip"] })
    .returning();
...
describe("GET /finished-good-slip/import/template", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import/template"));
    expect(res.status).toBe(401);
  });

  test("403 kalau login tapi tidak punya permission import.create", async () => {
    const email = `fgs-noperm-${runId}@test.local`;
    await signUp(email);
    const cookie = await signIn(email);
...
describe("POST /finished-good-slip/import/upload", () => {
  test("401 kalau tidak login (dengan file .xlsx asli, supaya bukan gagal validasi t.File duluan)", async () => {
    ...
    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import/upload", { method: "POST", body: form }));
    expect(res.status).toBe(401);
  });
});

describe("Ownership batch — user lain TIDAK BOLEH akses batch orang lain", () => {
  test("GET/POST /finished-good-slip/import/:batchId* dengan batchId milik user LAIN → 404, bukan bocor data", async () => {
    const owner = await createProvisionedUser(`fgs-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`fgs-attacker-${runId}@test.local`);
    ...
    const getRes = await testApp.handle(new Request(`http://localhost/finished-good-slip/import/${batch!.id}`, { headers: { cookie: attacker.cookie } }));
    expect(getRes.status).toBe(404);
    expect(((await getRes.json()) as { code: string }).code).toBe("BATCH_NOT_FOUND");

    const retryRes = await testApp.handle(
      new Request(`http://localhost/finished-good-slip/import/${batch!.id}/retry`, { method: "POST", headers: { cookie: attacker.cookie } }),
    );
    expect(retryRes.status).toBe(404);

    const ownerRes = await testApp.handle(new Request(`http://localhost/finished-good-slip/import/${batch!.id}`, { headers: { cookie: owner.cookie } }));
    expect(ownerRes.status).toBe(200);
  });
});

describe("POST /finished-good-slip/import/:batchId/confirm — validasi mapping", () => {
  test("400 MISSING_REQUIRED_FIELDS kalau kolom wajib belum di-mapping", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "mapping_pending" })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/finished-good-slip/import/${batch!.id}/confirm`, {
        method: "POST",
    ...
    // § itemNo/quantity/portion wajib BERSYARAT per baris (lewat finishedGoodSlipRowError), bukan di level mapping
    expect(body.fields).not.toContain("itemNo");
    expect(body.fields).not.toContain("quantity");
    expect(body.fields).not.toContain("portion");
  ...
  test("400 INVALID_MAPPING_FIELD kalau ada kolom di-mapping ke field yang tidak dikenal", async () => {
    const owner = await createProvisionedUser(`fgs-invalidfield-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "mapping_pending" })
      .returning();
...
describe("GET /finished-good-slip/import (list)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import"));
    expect(res.status).toBe(401);
  });

  test("cuma return batch milik subscription sendiri, urut terbaru dulu, dibatasi ?limit", async () => {
    const owner = await createProvisionedUser(`fgs-list-owner-${runId}@test.local`);
    const other = await createProvisionedUser(`fgs-list-other-${runId}@test.local`);
    ...
    for (const fileName of ["batch-1.xlsx", "batch-2.xlsx", "batch-3.xlsx"]) {
      await db.insert(importBatches).values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName, totalRows: 1, status: "completed" });
    }

    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import?limit=2", { headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(200);
  ...
  test("?offset melompati N batch terbaru, `total` tetap hitungan penuh — pola halaman Riwayat", async () => {
    const owner = await createProvisionedUser(`fgs-offset-${runId}@test.local`);
    for (const fileName of ["off-1.xlsx", "off-2.xlsx", "off-3.xlsx"]) {
      await db.insert(importBatches).values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName, totalRows: 1, status: "completed" });
    }

    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import?limit=2&offset=1", { headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(200);
    const body = (await res.json()) as { batches: { fileName: string }[]; total: number };
...
describe("PUT /finished-good-slip/import/:batchId/rows/:rowId — Edit Baris", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/finished-good-slip/import/00000000-0000-0000-0000-000000000000/rows/00000000-0000-0000-0000-000000000000", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`fgs-editrow-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`fgs-editrow-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
    const [row] = await db.insert(importBatchRows).values({ batchId: batch!.id, rowNumber: 1, rawData: {}, status: "failed" }).returning();

    const res = await testApp.handle(
      new Request(`http://localhost/finished-good-slip/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
  ...
  test("409 ROW_NOT_EDITABLE kalau baris statusnya bukan failed", async () => {
    const owner = await createProvisionedUser(`fgs-editrow-noteditable-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed", columnMapping })
      .returning();
  ...
  test("400 MISSING_REQUIRED_VALUES kalau field wajib dikosongkan, sukses (status pending) kalau lengkap", async () => {
    const owner = await createProvisionedUser(`fgs-editrow-save-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
  ...
  test("400 MISSING_REQUIRED_VALUES kalau 'Portion' dikosongkan — wajib bersamaan dengan Item No/Qty (§ finishedGoodSlipRowError)", async () => {
    const owner = await createProvisionedUser(`fgs-editrow-noportion-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
    const [row] = await db
      .insert(importBatchRows)
      .values({ batchId: batch!.id, rowNumber: 1, rawData: {}, status: "failed", errorMessage: "Portion wajib diisi" })
      .returning();
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/finished-good-slip/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { ...validRawData, Portion: "" } }),
      }),
    );
    expect(res.status).toBe(400);
    const body = (await res.json()) as { code: string; fields: string[] };
    expect(body.code).toBe("MISSING_REQUIRED_VALUES");
    expect(body.fields).toContain("portion");
  });
});

describe("PUT /finished-good-slip/import/:batchId/rows — Edit Bulk (Grid)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/finished-good-slip/import/00000000-0000-0000-0000-000000000000/rows", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`fgs-bulkedit-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`fgs-bulkedit-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping })
      .returning();
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/finished-good-slip/import/${batch!.id}/rows`, {
        method: "PUT",
        headers: { cookie: attacker.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rows: [] }),
      }),
  ...
  test("campuran baris valid & field wajib kosong, bukan gagalkan seluruh request", async () => {
    const owner = await createProvisionedUser(`fgs-bulkedit-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 2, status: "completed_with_errors", columnMapping })
      .returning();
...
describe("DELETE /finished-good-slip/import/:batchId — hapus riwayat lokal", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/finished-good-slip/import/00000000-0000-0000-0000-000000000000", { method: "DELETE" }));
    expect(res.status).toBe(401);
  });

  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`fgs-delete-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`fgs-delete-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/finished-good-slip/import/${batch!.id}`, { method: "DELETE", headers: { cookie: attacker.cookie } }));
    expect(res.status).toBe(404);
  });
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`fgs-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `fgs-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(new Request(`http://localhost/finished-good-slip/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }));
    expect(res.status).toBe(403);
  ...
  test("409 BATCH_BUSY kalau status processing", async () => {
    const owner = await createProvisionedUser(`fgs-delete-busy-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "test.xlsx", totalRows: 1, status: "processing" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/finished-good-slip/import/${batch!.id}`, { method: "DELETE", headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(409);
  ...
  test("berhasil hapus batch + baris (cascade), tercatat di audit log", async () => {
    const owner = await createProvisionedUser(`fgs-delete-ok-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "finished_good_slip", fileName: "hapus-saya.xlsx", totalRows: 1, status: "completed" })
      .returning();

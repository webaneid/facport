...
// § architecture-security.md §8, pola sama sales-quotation-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(salesOrderMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(salesOrderMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = salesOrderMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-sales-order.md — Fase 137. Kelanjutan Sales Quotation,
// auto-create customer+item (mirror Sales Invoice). TIDAK ADA "Batal
// Import" (bukan transaksi akuntansi, tidak ada GL impact) — route ini
// mirror PERSIS sales-quotation-import.route.ts, tanpa endpoint cancel.
export const salesOrderImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/sales-order/import/template",
    () => {
      const buffer = generateTemplateBuffer(salesOrderTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-sales-order.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "sales_order" },
  )
  .get(
    "/sales-order/import",
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "sales_order"));
  ...
  .post(
    "/sales-order/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = salesOrderMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "sales_order");
  ...
  .get(
    "/sales-order/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/sales-order/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-order/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-order/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .delete(
    ...
    async ({ params, user, subscription, set }) => {
      ...
      // § pola sama sales-quotation-import.route.ts — hapus riwayat
      // import HANYA pemilik Data Usaha SAAT INI.

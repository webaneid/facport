...
import { salesOrderImportRoute } from "./sales-order-import.route";
import { generateTemplateBuffer } from "../lib/excel";
import { createTestDataUsaha, createTestSeat } from "../lib/test-fixtures";

// § Fase 137 — mirror `sales-quotation-import.route.test.ts` (modul yang
// juga TIDAK punya endpoint cancel).
const runId = Date.now();
const testApp = new Elysia().mount(auth.handler).use(salesOrderImportRoute);
const columnMapping = {
  "Cust No": "customerNo",
...
async function signUp(email: string) {
  const res = await testApp.handle(
    new Request("http://localhost/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({ email, password: "TestPassword123!", name: "Sales Order Import Test" }),
    }),
  );
...
async function createProvisionedUser(email: string) {
  ...
  const [plan] = await db
    .insert(plans)
    .values({ name: `Sales Order Import Test Plan ${email}`, price: 1000, durationDays: 30, modules: ["sales_order"] })
    .returning();
...
describe("GET /sales-order/import/template", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/sales-order/import/template"));
    expect(res.status).toBe(401);
  });

  test("403 kalau login tapi tidak punya permission import.create", async () => {
    const email = `so-noperm-${runId}@test.local`;
    await signUp(email);
    const cookie = await signIn(email);

    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import/template", { headers: { cookie } }),
    );
    expect(res.status).toBe(403);
  });
});

describe("POST /sales-order/import/upload", () => {
  test("401 kalau tidak login (dengan file .xlsx asli, supaya bukan gagal validasi t.File duluan)", async () => {
    const buffer = generateTemplateBuffer([
      { column: "Cust No", required: true, example: "C.0001", description: "test" },
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import/upload", { method: "POST", body: form }),
    );
    expect(res.status).toBe(401);
  });
});
...
describe("Ownership batch — user lain TIDAK BOLEH akses batch orang lain", () => {
  test("GET/POST /sales-order/import/:batchId* dengan batchId milik user LAIN → 404, bukan bocor data", async () => {
    const ownerEmail = `so-owner-${runId}@test.local`;
    const attackerEmail = `so-attacker-${runId}@test.local`;
    ...
    const getRes = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}`, {
    ...
    const retryRes = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}/retry`, {
        method: "POST",
        headers: { cookie: attacker.cookie },
      }),
    );
    ...
    const ownerRes = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}`, { headers: { cookie: owner.cookie } }),
    );
    expect(ownerRes.status).toBe(200);
  });
});

describe("POST /sales-order/import/:batchId/confirm — validasi mapping", () => {
  test("400 MISSING_REQUIRED_FIELDS kalau kolom wajib belum di-mapping", async () => {
    const owner = await createProvisionedUser(`so-confirm-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}/confirm`, {
        method: "POST",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ columnMapping: { "Cust No": "customerNo" } }), // field wajib lain sengaja tidak di-mapping
      }),
    );
  ...
  test("400 INVALID_MAPPING_FIELD kalau ada kolom di-mapping ke field yang tidak dikenal", async () => {
    const owner = await createProvisionedUser(`so-invalidfield-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
...
describe("GET /sales-order/import (list)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/sales-order/import"));
    expect(res.status).toBe(401);
  });

  test("cuma return batch milik subscription sendiri, urut terbaru dulu, dibatasi ?limit", async () => {
    const owner = await createProvisionedUser(`so-list-owner-${runId}@test.local`);
    const other = await createProvisionedUser(`so-list-other-${runId}@test.local`);
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import?limit=2", { headers: { cookie: owner.cookie } }),
    );
  ...
  test("?offset melompati N batch terbaru, `total` tetap hitungan penuh — pola halaman Riwayat", async () => {
    const owner = await createProvisionedUser(`so-offset-${runId}@test.local`);
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import?limit=2&offset=1", { headers: { cookie: owner.cookie } }),
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as { batches: { fileName: string }[]; total: number };
...
describe("PUT /sales-order/import/:batchId/rows/:rowId — Edit Baris", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request(
        "http://localhost/sales-order/import/00000000-0000-0000-0000-000000000000/rows/00000000-0000-0000-0000-000000000000",
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`so-editrow-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`so-editrow-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
  ...
  test("409 ROW_NOT_EDITABLE kalau baris statusnya bukan failed", async () => {
    const owner = await createProvisionedUser(`so-editrow-noteditable-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
  ...
  test("400 MISSING_REQUIRED_VALUES kalau field wajib dikosongkan, sukses (status pending) kalau lengkap", async () => {
    const owner = await createProvisionedUser(`so-editrow-save-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const missingRes = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { "Cust No": "", "Trans Date": "05/09/2026", "Trans No": "SO-001", "Item No": "BRG-1", "Item Price": "1000", Qty: "1", "Unit Name": "Unit", "Branch Name": "JAKARTA" } }),
      }),
    );
    ...
    const okRes = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { "Cust No": "C.0001", "Trans Date": "05/09/2026", "Trans No": "SO-001", "Item No": "BRG-1", "Item Price": "1000", Qty: "1", "Unit Name": "Unit", "Branch Name": "JAKARTA" } }),
      }),
    );
...
describe("PUT /sales-order/import/:batchId/rows — Edit Bulk (Grid)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import/00000000-0000-0000-0000-000000000000/rows", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`so-bulkedit-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`so-bulkedit-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
  ...
  test("campuran baris valid & field wajib kosong, bukan gagalkan seluruh request", async () => {
    const owner = await createProvisionedUser(`so-bulkedit-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const validRawData = { "Cust No": "C.0001", "Trans Date": "05/09/2026", "Trans No": "SO-001", "Item No": "BRG-1", "Item Price": "1000", Qty: "1", "Unit Name": "Unit", "Branch Name": "JAKARTA" };
    const missingRawData = { ...validRawData, "Trans No": "SO-002", "Cust No": "" };
...
describe("DELETE /sales-order/import/:batchId — hapus riwayat lokal", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/sales-order/import/00000000-0000-0000-0000-000000000000", { method: "DELETE" }),
    );
    expect(res.status).toBe(401);
  });

  test("404 kalau batch milik user LAIN", async () => {
    const owner = await createProvisionedUser(`so-delete-owner-${runId}@test.local`);
    const attacker = await createProvisionedUser(`so-delete-attacker-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_order", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}`, { method: "DELETE", headers: { cookie: attacker.cookie } }),
    );
    expect(res.status).toBe(404);
  });
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`so-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_order", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `so-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }),
    );
    expect(res.status).toBe(403);
  ...
  test("409 BATCH_BUSY kalau status processing", async () => {
    const owner = await createProvisionedUser(`so-delete-busy-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_order", fileName: "test.xlsx", totalRows: 1, status: "processing" })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/sales-order/import/${batch!.id}`, { method: "DELETE", headers: { cookie: owner.cookie } }),
    );
    expect(res.status).toBe(409);
  ...
  test("berhasil hapus batch + baris (cascade), tercatat di audit log", async () => {
    const owner = await createProvisionedUser(`so-delete-ok-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_order", fileName: "hapus-saya.xlsx", totalRows: 1, status: "completed" })
      .returning();

...
import { receiveItemTemplateGuide } from "../lib/import-mapping/template-guide";
...
const VALID_FIELDS = new Set(Object.keys(receiveItemMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(receiveItemMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = receiveItemMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-receive-item.md — dokumen LANJUTAN dalam rantai
// procurement (TIDAK auto-create vendor/item, mirror Purchase Payment).
// TIDAK ADA "Batal Import" — route ini SESEDERHANA
// purchase-order-import.route.ts, tanpa endpoint cancel.
export const receiveItemImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/receive-item/import/template",
    () => {
      const buffer = generateTemplateBuffer(receiveItemTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-receive-item.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "receive_item" },
  )
  .get(
    "/receive-item/import",
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "receive_item"));
  ...
  .post(
    "/receive-item/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = receiveItemMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
  ...
  .get(
    "/receive-item/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/receive-item/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/receive-item/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing = receiveItemMapping.requiredFields.filter((field) => {
  ...
  .put(
    "/receive-item/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

...
import { salesReturnImportRoute } from "./sales-return-import.route";
import { generateTemplateBuffer } from "../lib/excel";
import { createTestDataUsaha, createTestSeat } from "../lib/test-fixtures";

// § Fase 124 — mirror `purchase-return-import.route.test.ts`. Beda
// utama: validasi `returnType` (§ `returnTypeRowError`) pakai susunan
// DELIVERY/INVOICE/INVOICE_DP/NO_INVOICE + field companion
// deliveryOrderNumber (bukan receiveItemNumber).
const runId = Date.now();
const testApp = new Elysia().mount(auth.handler).use(salesReturnImportRoute);
const columnMapping = {
...
async function signUp(email: string) {
  const res = await testApp.handle(
    new Request("http://localhost/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({ email, password: "TestPassword123!", name: "Sales Return Import Test" }),
    }),
  );
...
async function createProvisionedUser(email: string) {
  ...
  const [plan] = await db
    .insert(plans)
    .values({ name: `Sales Return Import Test Plan ${email}`, price: 1000, durationDays: 30, modules: ["sales_return"] })
    .returning();
...
describe("GET /sales-return/import/template", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/sales-return/import/template"));
  ...
  test("403 kalau login tapi tidak punya permission import.create", async () => {
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import/template", { headers: { cookie } }),
    );
    expect(res.status).toBe(403);
  });
});

describe("POST /sales-return/import/upload", () => {
  test("401 kalau tidak login (dengan file .xlsx asli, supaya bukan gagal validasi t.File duluan)", async () => {
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import/upload", { method: "POST", body: form }),
    );
    expect(res.status).toBe(401);
  });
});
...
describe("Ownership batch — user lain TIDAK BOLEH akses batch orang lain", () => {
  test("GET/POST /sales-return/import/:batchId* dengan batchId milik user LAIN → 404, bukan bocor data", async () => {
    ...
    const getRes = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}`, {
    ...
    const retryRes = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/retry`, {
        method: "POST",
        headers: { cookie: attacker.cookie },
      }),
    );
    ...
    const ownerRes = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}`, { headers: { cookie: owner.cookie } }),
    );
    expect(ownerRes.status).toBe(200);
  });
});

describe("POST /sales-return/import/:batchId/confirm — validasi mapping", () => {
  test("400 MISSING_REQUIRED_FIELDS kalau kolom wajib belum di-mapping", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/confirm`, {
        method: "POST",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ columnMapping: { "Customer No": "customerNo" } }), // field wajib lain sengaja tidak di-mapping
      }),
    );
    ...
    expect(body.fields).not.toContain("deliveryOrderNumber");
  });

  test("400 INVALID_MAPPING_FIELD kalau ada kolom di-mapping ke field yang tidak dikenal", async () => {
    const owner = await createProvisionedUser(`sr-invalidfield-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
...
describe("GET /sales-return/import (list)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/sales-return/import"));
    expect(res.status).toBe(401);
  });
  ...
  test("cuma return batch milik subscription sendiri, urut terbaru dulu, dibatasi ?limit", async () => {
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import?limit=2", { headers: { cookie: owner.cookie } }),
    );
  ...
  test("?offset melompati N batch terbaru, `total` tetap hitungan penuh — pola halaman Riwayat", async () => {
    ...
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import?limit=2&offset=1", { headers: { cookie: owner.cookie } }),
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as { batches: { fileName: string }[]; total: number };
...
describe("PUT /sales-return/import/:batchId/rows/:rowId — Edit Baris", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request(
        "http://localhost/sales-return/import/00000000-0000-0000-0000-000000000000/rows/00000000-0000-0000-0000-000000000000",
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: attacker.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { "Customer No": "C.0001" } }),
      }),
    );
  ...
  test("400 MISSING_REQUIRED_VALUES kalau field wajib dikosongkan, sukses (status pending) kalau lengkap", async () => {
    ...
    const missingRes = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { ...validRowData, "Customer No": "" } }),
      }),
    );
  ...
  test("400 MISSING_REQUIRED_VALUES ['deliveryOrderNumber'] kalau Return Type=DELIVERY tapi Delivery Order No kosong", async () => {
    const owner = await createProvisionedUser(`sr-editrow-returntype-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const [row] = await db
      .insert(importBatchRows)
      .values({ batchId: batch!.id, rowNumber: 1, rawData: {}, status: "failed", errorMessage: "Delivery Order No wajib" })
      .returning();
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { ...validRowData, "Return Type": "DELIVERY", "Delivery Order No": "" } }),
      }),
    );
    expect(res.status).toBe(400);
    const body = (await res.json()) as { code: string; fields: string[] };
    expect(body.code).toBe("MISSING_REQUIRED_VALUES");
    expect(body.fields).toContain("deliveryOrderNumber");
  });
});

describe("PUT /sales-return/import/:batchId/rows — Edit Bulk (Grid)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import/00000000-0000-0000-0000-000000000000/rows", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}/rows`, {
        method: "PUT",
        headers: { cookie: attacker.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rows: [] }),
      }),
  ...
  test("campuran baris valid & Return Type tidak konsisten, bukan gagalkan seluruh request", async () => {
    const owner = await createProvisionedUser(`sr-bulkedit-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const invalidRowData = { ...validRowData, "Return Type": "INVOICE", "Invoice No": "" };
    ...
    expect(body.errors[0]!.fields).toContain("invoiceNumber");
...
describe("DELETE /sales-return/import/:batchId — hapus riwayat lokal", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/sales-return/import/00000000-0000-0000-0000-000000000000", { method: "DELETE" }),
    );
    expect(res.status).toBe(401);
  });
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_return", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}`, { method: "DELETE", headers: { cookie: attacker.cookie } }),
    );
    expect(res.status).toBe(404);
  });
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`sr-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_return", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `sr-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }),
    );
    expect(res.status).toBe(403);
  ...
  test("409 BATCH_BUSY kalau status processing", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_return", fileName: "test.xlsx", totalRows: 1, status: "processing" })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/sales-return/import/${batch!.id}`, { method: "DELETE", headers: { cookie: owner.cookie } }),
    );
    expect(res.status).toBe(409);
  ...
  test("berhasil hapus batch + baris (cascade), tercatat di audit log", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "sales_return", fileName: "hapus-saya.xlsx", totalRows: 1, status: "completed" })
      .returning();

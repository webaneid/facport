...
import { purchaseOrderMapping } from "../lib/import-mapping/purchase-order.mapping";
import { purchaseOrderTemplateGuide } from "../lib/import-mapping/template-guide";
import { boss, JOBS } from "../lib/queue";
import { checkTrialRowBudget } from "../lib/trial";

// § architecture-security.md §8, pola sama sales-receipt-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(purchaseOrderMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(purchaseOrderMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = purchaseOrderMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-purchase-order.md — titik AWAL rantai procurement,
// auto-create vendor+item (mirror Purchase Invoice). TIDAK ADA "Batal
// Import" (bukan transaksi akuntansi, 2 PO nominal sama bukan duplikat)
// — route ini SESEDERHANA sales-receipt-import.route.ts, tanpa endpoint
// cancel.
export const purchaseOrderImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/purchase-order/import/template",
    () => {
      const buffer = generateTemplateBuffer(purchaseOrderTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-purchase-order.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "purchase_order" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "purchase_order"));
  ...
  .post(
    "/purchase-order/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = purchaseOrderMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "purchase_order");
  ...
  .get(
    "/purchase-order/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/purchase-order/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/purchase-order/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing = purchaseOrderMapping.requiredFields.filter((field) => {
  ...
  .put(
    "/purchase-order/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

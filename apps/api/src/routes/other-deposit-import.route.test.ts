...
import { otherDepositImportRoute } from "./other-deposit-import.route";
...
const testApp = new Elysia().mount(auth.handler).use(otherDepositImportRoute);
...
async function signUp(email: string) {
  const res = await testApp.handle(
    new Request("http://localhost/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({ email, password: "TestPassword123!", name: "Other Deposit Import Test" }),
    }),
  );
...
async function createProvisionedUser(email: string) {
  ...
  const [plan] = await db
    .insert(plans)
    .values({ name: `Other Deposit Import Test Plan ${email}`, price: 1000, durationDays: 30, modules: ["other_deposit"] })
    .returning();
...
describe("GET /other-deposit/import/template", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import/template"));
  ...
  test("403 kalau login tapi tidak punya permission import.create", async () => {
    ...
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import/template", { headers: { cookie } }));
    expect(res.status).toBe(403);
  });
});

describe("POST /other-deposit/import/upload", () => {
  test("401 kalau tidak login (dengan file .xlsx asli, supaya bukan gagal validasi t.File duluan)", async () => {
    ...
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import/upload", { method: "POST", body: form }));
    expect(res.status).toBe(401);
  });
});

describe("Ownership batch — user lain TIDAK BOLEH akses batch orang lain", () => {
  test("GET/POST /other-deposit/import/:batchId* dengan batchId milik user LAIN → 404, bukan bocor data", async () => {
    ...
    const getRes = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}`, { headers: { cookie: attacker.cookie } }),
    );
    expect(getRes.status).toBe(404);
    expect(((await getRes.json()) as { code: string }).code).toBe("BATCH_NOT_FOUND");

    const retryRes = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}/retry`, { method: "POST", headers: { cookie: attacker.cookie } }),
    );
    expect(retryRes.status).toBe(404);

    const ownerRes = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}`, { headers: { cookie: owner.cookie } }),
    );
    expect(ownerRes.status).toBe(200);
  });
});

describe("POST /other-deposit/import/:batchId/confirm — validasi mapping", () => {
  test("400 MISSING_REQUIRED_FIELDS kalau kolom wajib belum di-mapping", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}/confirm`, {
        method: "POST",
  ...
  test("400 INVALID_MAPPING_FIELD kalau field tidak dikenal", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 1, status: "mapping_pending" })
      .returning();
...
describe("GET /other-deposit/import (list)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import"));
    expect(res.status).toBe(401);
  });
  ...
  test("cuma return batch milik subscription sendiri, urut terbaru dulu", async () => {
    ...
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import?limit=10", { headers: { cookie: owner.cookie } }));
...
describe("PUT /other-deposit/import/:batchId/rows/:rowId — Edit Baris", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request(
        "http://localhost/other-deposit/import/00000000-0000-0000-0000-000000000000/rows/00000000-0000-0000-0000-000000000000",
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
...
describe("PUT /other-deposit/import/:batchId/rows — Edit Bulk (Grid)", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(
      new Request("http://localhost/other-deposit/import/00000000-0000-0000-0000-000000000000/rows", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 1, status: "completed_with_errors", columnMapping: fullColumnMapping })
      .returning();

    const res = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}/rows`, {
        method: "PUT",
        headers: { cookie: attacker.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rows: [] }),
      }),
  ...
  test("campuran baris valid & field wajib kosong, bukan gagalkan seluruh request", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 2, status: "completed_with_errors", columnMapping: fullColumnMapping })
      .returning();
...
describe("DELETE /other-deposit/import/:batchId — hapus riwayat lokal", () => {
  test("401 kalau tidak login", async () => {
    const res = await testApp.handle(new Request("http://localhost/other-deposit/import/00000000-0000-0000-0000-000000000000", { method: "DELETE" }));
    expect(res.status).toBe(401);
  });
  ...
  test("404 kalau batch milik user LAIN", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/other-deposit/import/${batch!.id}`, { method: "DELETE", headers: { cookie: attacker.cookie } }));
    expect(res.status).toBe(404);
  });
  ...
  test("403 DELETE_OWNER_ONLY kalau yang hapus MEMBER (bukan pemilik Data Usaha), walau seat-nya aktif di Data Usaha yang sama", async () => {
    const owner = await createProvisionedUser(`op-delete-memberowner-${runId}@test.local`);
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 1, status: "completed" })
      .returning();

    const memberEmail = `op-delete-member-${runId}@test.local`;
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/other-deposit/import/${batch!.id}`, { method: "DELETE", headers: { cookie: memberCookie } }),
    );
    expect(res.status).toBe(403);
  ...
  test("409 BATCH_BUSY kalau status processing", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "test.xlsx", totalRows: 1, status: "processing" })
      .returning();

    const res = await testApp.handle(new Request(`http://localhost/other-deposit/import/${batch!.id}`, { method: "DELETE", headers: { cookie: owner.cookie } }));
    expect(res.status).toBe(409);
  ...
  test("berhasil hapus batch + baris (cascade), tercatat di audit log", async () => {
    ...
    const [batch] = await db
      .insert(importBatches)
      .values({ userId: owner.userId, subscriptionId: owner.subscriptionId, module: "other_deposit", fileName: "hapus-saya.xlsx", totalRows: 1, status: "completed" })
      .returning();

...
import { salesQuotationMapping } from "../lib/import-mapping/sales-quotation.mapping";
import { salesQuotationTemplateGuide } from "../lib/import-mapping/template-guide";
...
const VALID_FIELDS = new Set(Object.keys(salesQuotationMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(salesQuotationMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = salesQuotationMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-sales-quotation.md — dokumen PALING AWAL rantai
// penjualan, auto-create customer+item (mirror Sales Invoice). TIDAK
// ADA "Batal Import" (bukan transaksi akuntansi, tidak ada GL impact) —
// route ini sesederhana purchase-order-import.route.ts, tanpa endpoint
// cancel.
export const salesQuotationImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/sales-quotation/import/template",
    () => {
      const buffer = generateTemplateBuffer(salesQuotationTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-sales-quotation.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "sales_quotation" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "sales_quotation"));
  ...
  .post(
    "/sales-quotation/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = salesQuotationMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "sales_quotation");
  ...
  .get(
    "/sales-quotation/import/:batchId",
  ...
  .get(
    "/sales-quotation/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/sales-quotation/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-quotation/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing = salesQuotationMapping.requiredFields.filter((field) => {
  ...
  .put(
    "/sales-quotation/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

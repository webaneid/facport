...
// § architecture-security.md §8, pola sama purchase-payment-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(journalVoucherMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(journalVoucherMapping.defaultColumnMap).find(
      (defaultCol) => defaultCol.toLowerCase() === normalized,
    );
    if (match) suggestion[col] = journalVoucherMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}
...
export const journalVoucherImportRoute = new Elysia()
  ...
  .get(
    ...
    () => {
      const buffer = generateTemplateBuffer(journalVoucherTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-jurnal-umum.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "journal_voucher" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "journal_voucher"));
  ...
  .post(
    "/journal-voucher/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "journal_voucher");
      if (!scopeCheck.ok) {
        set.status = 409;
  ...
  .get(
    "/journal-voucher/import/:batchId",
  ...
  .get(
    "/journal-voucher/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/journal-voucher/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  // `failed` yang boleh diedit. Validasi balance debit=kredit TIDAK
  // diulang di sini (cukup cek field wajib TERISI, sama seperti modul
  // lain) — kalau nominalnya tidak seimbang, `buildJournalVoucherPayload()`
  // di worker akan menolak lagi saat retry dengan pesan jelas, user
  // edit ulang, konsisten dengan pola PI (validasi bisnis di retry,
  // bukan di endpoint edit).
  .put(
    "/journal-voucher/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  // `errors`, BUKAN gagalkan seluruh request). Format wide/tall
  // dideteksi SEKALI dari `columnMapping` batch, dipakai buat SEMUA
  // baris (sama alasan endpoint per-baris di atas).
  .put(
    "/journal-voucher/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

...
import { inventoryAdjustmentMapping, itemAdjustmentTypeRowError } from "../lib/import-mapping/inventory-adjustment.mapping";
import { inventoryAdjustmentTemplateGuide } from "../lib/import-mapping/template-guide";
...
const VALID_FIELDS = new Set(Object.keys(inventoryAdjustmentMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(inventoryAdjustmentMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = inventoryAdjustmentMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-inventory-adjustment.md — TIDAK auto-create item (mirror
// Item Transfer/Receive Item), TIDAK ADA "Batal Import". `itemAdjustmentTypeRowError`
// divalidasi TERPISAH dari `requiredFields` biasa (§ mapping file) di
// KEDUA endpoint edit baris gagal, mirror `itemTransferTypeRowError`.
export const inventoryAdjustmentImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/inventory-adjustment/import/template",
    () => {
      const buffer = generateTemplateBuffer(inventoryAdjustmentTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-inventory-adjustment.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "inventory_adjustment" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "inventory_adjustment"));
    ...
    },
    {
      ...
      moduleAccess: "inventory_adjustment",
      query: t.Object({
  ...
  .post(
    "/inventory-adjustment/import/upload",
  ...
  .post(
    "/inventory-adjustment/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = inventoryAdjustmentMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "inventory_adjustment");
  ...
  .get(
    "/inventory-adjustment/import/:batchId",
  ...
  .get(
    "/inventory-adjustment/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/inventory-adjustment/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/inventory-adjustment/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing: string[] = inventoryAdjustmentMapping.requiredFields.filter((field) => {
      ...
      missing.push(...itemAdjustmentTypeRowError(body.rawData, columnMapping));
      if (missing.length > 0) {
        set.status = 400;
  ...
  .put(
    "/inventory-adjustment/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      for (const item of body.rows) {
        ...
        missing.push(...itemAdjustmentTypeRowError(item.rawData, columnMapping));
        if (missing.length > 0) {

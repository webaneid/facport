...
import { salesReceiptMapping } from "../lib/import-mapping/sales-receipt.mapping";
import { salesReceiptTemplateGuide } from "../lib/import-mapping/template-guide";
...
const VALID_FIELDS = new Set(Object.keys(salesReceiptMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(salesReceiptMapping.defaultColumnMap).find(
      (defaultCol) => defaultCol.toLowerCase() === normalized,
    );
    if (match) suggestion[col] = salesReceiptMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}
...
export const salesReceiptImportRoute = new Elysia()
  ...
  .get(
    ...
    () => {
      const buffer = generateTemplateBuffer(salesReceiptTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-sales-receipt.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "sales_receipt" },
  )
  .get(
    ...
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "sales_receipt"));
  ...
  .post(
    "/sales-receipt/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = salesReceiptMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "sales_receipt");
      if (!scopeCheck.ok) {
        set.status = 409;
  ...
  .get(
    "/sales-receipt/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/sales-receipt/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-receipt/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing = salesReceiptMapping.requiredFields.filter((field) => {
  ...
  .put(
    "/sales-receipt/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

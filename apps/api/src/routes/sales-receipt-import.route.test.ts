...
describe("PUT /sales-receipt/import/:batchId/rows/:rowId — Edit Baris", () => {
  ...
  test("400 MISSING_REQUIRED_VALUES kalau field wajib dikosongkan, sukses (status pending) kalau lengkap", async () => {
    ...
    const columnMapping = { "No Pelanggan": "customerNo", "Akun Bank/Kas": "bankNo", "Jumlah Bayar": "chequeAmount", "Tanggal": "transDate", "No Faktur": "invoiceNo" };
    const [batch] = await db
      .insert(importBatches)
      .values({
        userId: owner.userId,
    ...
    const missingRes = await testApp.handle(
      new Request(`http://localhost/sales-receipt/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { "No Pelanggan": "C-0001", "Akun Bank/Kas": "1-10200", "Jumlah Bayar": "1000000", "Tanggal": "05/09/2026", "No Faktur": "" } }),
      }),
    );
    ...
    const okRes = await testApp.handle(
      new Request(`http://localhost/sales-receipt/import/${batch!.id}/rows/${row!.id}`, {
        method: "PUT",
        headers: { cookie: owner.cookie, "Content-Type": "application/json" },
        body: JSON.stringify({ rawData: { "No Pelanggan": "C-0001", "Akun Bank/Kas": "1-10200", "Jumlah Bayar": "1000000", "Tanggal": "05/09/2026", "No Faktur": "SI-001" } }),
      }),
    );
  ...
  test("campuran: baris valid tersimpan (pending), baris field wajib kosong & baris status bukan failed dicatat di errors, bukan gagalkan seluruh request", async () => {
    ...
    const res = await testApp.handle(
      new Request(`http://localhost/sales-receipt/import/${batch!.id}/rows`, {
        ...
        body: JSON.stringify({
          rows: [
            { id: validRow!.id, rawData: { "No Pelanggan": "C-0001", "Akun Bank/Kas": "1-10200", "Jumlah Bayar": "1000000", Tanggal: "05/09/2026", "No Faktur": "SI-001" } },
            { id: missingRow!.id, rawData: { "No Pelanggan": "C-0001", "Akun Bank/Kas": "1-10200", "Jumlah Bayar": "1000000", Tanggal: "05/09/2026", "No Faktur": "" } },

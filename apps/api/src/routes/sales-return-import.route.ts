...
// § architecture-security.md §8, pola sama purchase-return-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(salesReturnMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(salesReturnMapping.defaultColumnMap).find((defaultCol) => defaultCol.toLowerCase() === normalized);
    if (match) suggestion[col] = salesReturnMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-sales-return.md — dokumen LANJUTAN (TIDAK auto-create
// customer/item, mirror Purchase Return). TIDAK ADA "Batal Import" —
// `returnTypeRowError` divalidasi TERPISAH dari requiredFields generik
// (mirror pola Purchase Return) karena field companion-nya
// (invoiceNumber/deliveryOrderNumber) WAJIB KONDISIONAL tergantung
// nilai returnType.
export const salesReturnImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/sales-return/import/template",
    () => {
      const buffer = generateTemplateBuffer(salesReturnTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-sales-return.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "sales_return" },
  )
  .get(
    "/sales-return/import",
    async ({ subscription, query }) => {
      ...
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "sales_return"));
  ...
  .post(
    "/sales-return/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = salesReturnMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
      ...
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "sales_return");
  ...
  .get(
    "/sales-return/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/sales-return/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-return/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .put(
    "/sales-return/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;

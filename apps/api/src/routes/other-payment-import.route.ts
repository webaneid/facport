...
import { otherPaymentMapping } from "../lib/import-mapping/other-payment.mapping";
import { otherPaymentTemplateGuide } from "../lib/import-mapping/template-guide";
import { boss, JOBS } from "../lib/queue";
import { checkTrialRowBudget } from "../lib/trial";

// § architecture-security.md §8, pola sama journal-voucher-import.route.ts
const ALLOWED_MIME = [
...
const VALID_FIELDS = new Set(Object.keys(otherPaymentMapping.fieldToAccuratePath));

function suggestMapping(excelColumns: string[]): Record<string, string> {
  const suggestion: Record<string, string> = {};
  for (const col of excelColumns) {
    const normalized = col.trim().toLowerCase();
    const match = Object.keys(otherPaymentMapping.defaultColumnMap).find(
      (defaultCol) => defaultCol.toLowerCase() === normalized,
    );
    if (match) suggestion[col] = otherPaymentMapping.defaultColumnMap[match]!;
  }
  return suggestion;
}

// § architecture-other-payment.md — pembayaran bank/kas untuk beban
// langsung, TANPA faktur/vendor. Baris dengan "Trans No" sama digabung
// jadi 1 transaksi (N akun beban), diproses PER GRUP (pola sama Jurnal
// Umum/Sales Receipt), bukan per-baris. TIDAK ADA fitur "Batal Import"
// (konsisten pola modul serupa, § architecture doc).
export const otherPaymentImportRoute = new Elysia()
  .use(permissionPlugin)
  .use(subscriptionGatePlugin)
  .get(
    "/other-payment/import/template",
    () => {
      const buffer = generateTemplateBuffer(otherPaymentTemplateGuide);
      return new Response(new Uint8Array(buffer), {
        headers: {
          "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition": 'attachment; filename="template-other-payment.xlsx"',
        },
      });
    },
    { permission: "import.create", moduleAccess: "other_payment" },
  )
  .get(
    "/other-payment/import",
    async ({ subscription, query }) => {
      const limit = query.limit ?? 10;
      const offset = query.offset ?? 0;
      const where = and(eq(importBatches.subscriptionId, subscription.id), eq(importBatches.module, "other_payment"));
  ...
  .post(
    "/other-payment/import/:batchId/confirm",
    async ({ params, body, subscription, set }) => {
      ...
      const missing = otherPaymentMapping.requiredFields.filter((f) => !mappedFields.has(f));
      if (missing.length > 0) {
        set.status = 400;
        return { code: "MISSING_REQUIRED_FIELDS", fields: missing };
      }

      // § Fase 43 — trial dibatasi jumlah baris berhasil-import.
      // § Fase 142 — koneksi Accurate WAJIB sudah punya scope modul ini (pesan jelas SEBELUM job dijadwalkan).
      const scopeCheck = await checkSubscriptionScopes(subscription.id, "other_payment");
      if (!scopeCheck.ok) {
        set.status = 409;
  ...
  .get(
    "/other-payment/import/:batchId/failed-rows/export",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  .post(
    "/other-payment/import/:batchId/retry",
    async ({ params, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
  ...
  // § edit baris GAGAL langsung di aplikasi, pola sama
  // `journal-voucher-import.route.ts` — TANPA pengecualian XOR (modul
  // ini tidak punya field semacam itu, semua requiredFields dicek apa
  // adanya).
  .put(
    "/other-payment/import/:batchId/rows/:rowId",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
      ...
      const columnMapping = (batch.columnMapping ?? {}) as Record<string, string>;
      const missing: string[] = otherPaymentMapping.requiredFields.filter((field) => {
  ...
  // § versi BULK (JAMAK "/rows"), dipakai grid edit ala Excel — mirror
  // `journal-voucher-import.route.ts`.
  .put(
    "/other-payment/import/:batchId/rows",
    async ({ params, body, subscription, set }) => {
      const [batch] = await db.select().from(importBatches).where(eq(importBatches.id, params.batchId));
      if (!batch || batch.subscriptionId !== subscription.id) {
        set.status = 404;
